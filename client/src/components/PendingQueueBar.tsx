...
import { type PendingRecord } from "@/hooks/useOfflineQueue";

const WARN_MS = 30 * 60 * 1000;  // 30 min → amber
...
interface PendingQueueBarProps {
  ...
  onRetry: (item: PendingRecord) => void;
...
export default function PendingQueueBar({
  ...
  onDiscard,
  onSyncAll,
}: PendingQueueBarProps) {
  const [expanded, setExpanded] = useState(false);

  if (records.length === 0) return null;

  const failedCount = records.filter((r) => r.status === "failed").length;
  const staleCount = records.filter((r) => Date.now() - r.queuedAt >= WARN_MS).length;
      ...
      >
        ...
        <div className={`mx-3 mb-1 rounded-xl shadow-lg flex items-center overflow-hidden ${staleCount > 0 ? "bg-orange-500 dark:bg-orange-600" : "bg-amber-500 dark:bg-amber-600"}`}>
          ...
          >
            ...
            <span>
              {records.length} record{records.length !== 1 ? "s" : ""} pending sync
              {staleCount > 0 && (
