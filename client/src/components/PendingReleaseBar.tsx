...
interface PendingReleaseBarProps {
...
export default function PendingReleaseBar({
  items,
  syncingIds,
  onRetry,
  onDiscard,
  onSyncAll,
}: PendingReleaseBarProps) {
      ...
      >
          ...
          <div className="mx-3 mb-1 rounded-xl border border-amber-300 bg-amber-50 dark:bg-amber-950/90 dark:border-amber-700 shadow-lg overflow-hidden">
            <div className="max-h-56 overflow-y-auto divide-y divide-amber-200 dark:divide-amber-800">
              {items.map((item, idx) => {
                ...
                const label = item.type === "release"
                  ? (item.captureDogId ?? item.dogId ?? "Release")
                  : (item.dogId ?? "Plan add");
                const photo = item.type === "release" ? item.photo3Base64 : item.photo2Base64;
                return (
                  <div
                    key={item.queueId}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-amber-500 dark:text-amber-500">
                        {item.type === "release" ? "Release" : "Plan add"} · {formatAge(item.queuedAt)}
                        {item.status === "failed" && (
        ...
        <div className="mx-3 mb-1 rounded-xl bg-amber-500 dark:bg-amber-600 shadow-lg flex items-center overflow-hidden">
          ...
          >
            ...
            <span>
              {items.length} release{items.length !== 1 ? "s" : ""} pending sync
              {failedCount > 0 && (
